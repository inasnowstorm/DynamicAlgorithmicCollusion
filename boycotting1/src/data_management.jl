module data_management
    ...
    function iterated_write(model::Main.structs.model)
        for i in 1:length(model.consumers)
            CSV.write(string("data/consumer_data", i, ".csv"), model.consumers[i].data)
        end
        for i in 1:length(model.firms)
            
            CSV.write(string("data/firm_data", i, ".csv"), model.firms[i].data)
        end
        CSV.write("data/world_data.csv", model.data)
    end
