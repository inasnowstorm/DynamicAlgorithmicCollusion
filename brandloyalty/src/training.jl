module training
    ...
    function simulate_model(model::Main.structs.model)::Main.structs.model
        ...
        for t in 1:model.tmax
            ...
            model.consumers = move_consumers(model, d)
            model.s = a
            model.t += 1
        end
        return model
    ...
    function move_consumers(model::Main.structs.model, d::Array{Int64,1})::Array{Main.structs.consumer,1}
        for i in eachindex(d)
            if d[i] != 0
                ...
                if model.consumers[i].location < 0
                    model.consumers[i].location = 1 + model.consumers[i].location
