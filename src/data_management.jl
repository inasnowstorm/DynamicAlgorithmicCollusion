module data_management
    ...
    # writes out all the data in the model
    ...
    # averages the data in the model according to given bins
    function average_data(model::Main.structs.model, bins::Vector{Int64})::Main.structs.model
        for i in 1:length(model.consumers)
            ...
            for j in 1:(length(bins)/2)
                push!(new_data,
                (mean(model.consumers[i].data.prices[bins[Int64(2j-1)]:bins[Int64(2j)]]),
                mean(model.consumers[i].data.profits[bins[Int64(2j-1)]:bins[Int64(2j)]]),
                mode(model.consumers[i].data.firm[bins[Int64(2j-1)]:bins[Int64(2j)]]),
                mean(model.consumers[i].data.location[bins[Int64(2j-1)]:bins[Int64(2j)]])))
            end
        ...
        for i in 1:length(model.firms)
            ...
            for j in 1:(length(bins)/2)
                push!(new_data,
                (mean(model.firms[i].data.prices[bins[Int64(2j-1)]:bins[Int64(2j)]]),
                mean(model.firms[i].data.profits[bins[Int64(2j-1)]:bins[Int64(2j)]])))
            end
        ...
        for j in 1:(length(bins)/2)
            push!(new_data,
            (mean(model.data.average_prices[bins[Int64(2j-1)]:bins[Int64(2j)]]),
            mean(model.data.total_profits[bins[Int64(2j-1)]:bins[Int64(2j)]])))
        end
        model.data = new_data
        return model
    end
